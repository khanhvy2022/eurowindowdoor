...
for (let i = 0; i < files.length; i++) {
  const slug = decodeURIComponent(files[i]).replace(/\\s+$/g, '').trim(); // the file name is URL encoded for one of them
  ...
  for (const line of lines) {
    if (line.includes('Chia sẻ: [ Facebook ]')) {
      inContent = false;
      break;
    }
    if (inContent) {
    ...
    if (line.match(new RegExp('\\\\d{2}/\\\\d{2}/\\\\d{4}\\\\s+\\\\[Tin dự án\\\\]'))) {
      inContent = true;
    }
  }
  ...
  newsTsContent = newsTsContent.substring(0, insertPos) + '\\n' + objStr + ',', + newsTsContent.substring(insertPos);
}

fs.writeFileSync(newsTsPath, newsTsContent, 'utf8');
console.log('Successfully added remaining 6 articles to news.ts');
