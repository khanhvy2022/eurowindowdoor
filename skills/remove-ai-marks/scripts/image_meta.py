...
def inspect_jpeg(data: bytes) -> tuple[bool, bool, list[str]]:
    ...
    while i + 4 <= n:
        ...
        if marker in (0xE1, 0xE2, 0xED, 0xEE, 0xEB):
            ...
            if hits:
                has_ai = True
                if any(h.lower() in ("c2pa", "contentcredentials", "jumb", "contentauth") for h in hits):
                    has_c2pa = True
    ...
    if exiftool:
        try:
            ...
            interesting = [
                ...
                if re.search(r"c2pa|content.?credential|AIGC|digitalSource|XMP|EXIF|IPTC|jumb", line, re.I)
            ]
...
def clean_image(path: Path, dest: Path, *, strip_all_metadata: bool = True) -> dict[str, Any]:
    data = path.read_bytes()
    fmt = detect_format(data)
    if fmt == "png":
